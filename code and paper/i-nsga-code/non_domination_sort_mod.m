function [f,Cmetric] = non_domination_sort_mod(x, M, V)
...
%         average_distance = sum(sorted_based_on_objective(:,V + i))/length(index_of_objectives);
          for j = 2 : length(index_of_objectives) - 1%循环集合中除了第一个和最后一个的个体
             ...
             if (f_max - f_min == 0)
                 ...
                 y(index_of_objectives(j),M + V + 1 + i) = ...
                       (next_obj - previous_obj)/[(f_max - f_min)*length(index_of_objectives)];
             end
          end
    end
    distance = [];
    ...
    W = [0.4,0.4,0.2];
    for i = 1 : M
       % distance(:,1) = distance(:,1) + W(i)*y(:,M + V + 1 + i); 
